...
/// and `AsyncWriteExt` traits from the source and the destination.  
...
/// Relay policy is meant to protect upstream and proxy servers from
...
impl Relay {
    ...
    ///     upstream_relay.relay_data(downstream_recv, upstream_send).await
    /// });
    /// let downstream = tokio::spawn(async move {
    ///     downstream_relay.relay_data(upstream_recv, downstream_send).await
    /// });
    /// let downstream_stats = upstream.await??;
    /// let upstream_stats = downstream.await??;
    /// ```
...
mod test_relay {
    ...
    async fn test_relay_ok() {
        ...
        let (downstream_recv, _) = io::split(reader);
        let (_, upstream_send) = io::split(writer);

        let result = relay.relay_data(downstream_recv, upstream_send).await;
